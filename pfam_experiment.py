...
flags.DEFINE_integer('lens_shuffle_seed', 0, 'Random seed used for lens training data batching.')
flags.DEFINE_integer('lens_sample_random_state', 0, 'Random state used for lens training data sampling.')
flags.DEFINE_integer('knn_shuffle_seed', 1, 'Random seed used for KNN data batching.')
flags.DEFINE_integer('knn_sample_random_state', 1, 'Random state used for KNN data sampling.')
...
def main(_):
	...
	if FLAGS.use_transformer:
		assert(FLAGS.encoder_fn_name is 'transformer'), 'encoder_fn_name must be \'transformer\' if \'use_transformer\' is True'
	...
	def measure_nearest_neighbor_performance(datum, accuracy_label, encoder, family_accessions, batch_size, 
											 train_samples, shuffle_seed, sample_random_state):
		...
		results = pfam_nearest_neighbors_classification(encoder=encoder, 
					                                    ...
					                                    sample_random_state=sample_random_state)[0]
		accuracy = results['1-nn accuracy']
		datum[accuracy_label] = accuracy
	...
	datum = measure_nearest_neighbor_performance(datum=datum, 
												 accuracy_label='train_knn_accuracy_untrained_lens_1_knn_train_samples',
												 encoder=embedding_optimizer.target,
	...
	for knn_train_samples in knn_train_samples_:
		...
		datum = measure_nearest_neighbor_performance(datum=datum, 
													 accuracy_label='test_knn_accuracy_untrained_lens_' + str(knn_train_samples) + '_knn_train_samples',
													 encoder=embedding_optimizer.target,
	...
	for i in range(FLAGS.measurements):
		...
		train_batches, train_indexes = create_pfam_batches(family_accessions=lens_knn_train_family_accessions,
														   ...
														   sample_random_state=FLAGS.sample_random_state)
		...
		datum = measure_nearest_neighbor_performance(datum=datum, 
													 accuracy_label='train_knn_accuracy_trained_lens_1_knn_train_samples' + '_measurement_' + str(i),
													 encoder=embedding_optimizer.target,
													 family_accessions=lens_knn_train_family_accessions,
													 batch_size=FLAGS.knn_batch_size,
													 train_samples=1,
		...
		for knn_train_samples in knn_train_samples_:
			...
			datum = measure_nearest_neighbor_performance(datum=datum, 
														 accuracy_label='test_knn_accuracy_trained_lens_' + str(knn_train_samples) + '_knn_train_samples' + '_measurement_' + str(i),
														 encoder=embedding_optimizer.target,
														 family_accessions=knn_test_family_accessions,
														 batch_size=FLAGS.knn_batch_size,
														 train_samples=knn_train_samples,
