...
### Pretraining
We also measure the effect that pretraining has on the performance of a language model encoder. There has been a great deal of interest in measuring the degree to which pretraining protein language models improves their performance on downstream tasks (https://arxiv.org/pdf/1906.08230.pdf). Our results indicate that pretraining offers a substantial boost in performance on the family classification task. We use transformer language models, specifically BERT models similar to to the the ProGen model (https://www.biorxiv.org/content/10.1101/2020.03.07.982272v2.full.pdf) and the models used by FAIR (https://www.biorxiv.org/content/10.1101/622803v2.full.pdf). The models are implemented in jax/flax (https://github.com/google-research/google-research/tree/master/protein_lm) and pretrained on the trembl protein corpus (ADD REFERENCE).
