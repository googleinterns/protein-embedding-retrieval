...
def load_transformer_model(ckpt_dir, model_cls, domain=None, bucket_name='gs://sequin-public'):
  """Loads a model from directory."""
  
  if domain is None:
    domain = data.protein_domain
        config_path = os.path.join(bucket_name, ckpt_dir, 'config.gin')
  with gin.config_scope('load_model'):
    ...
    model.load_checkpoint(os.path.join(bucket_name, ckpt_dir))
