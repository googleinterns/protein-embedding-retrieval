...
def main(_):
	...
	with gcsfs.open('step_1.txt', 'w') as f:
		f.write('GO')
	...
	for knn_train_samples in knn_train_samples_:
		...
		with gcsfs.open('step_2' + str(knn_train_samples) + '.txt', 'w') as f:
			f.write('GO')
	...
	with gcsfs.open('step_3.txt', 'w') as f:
		f.write('GO')
	...
	for i in range(FLAGS.measurements):
		...
		with gcsfs.open('step_4' + str(i) + '.txt', 'w') as f:
			f.write('GO')
		...
		with gcsfs.open('step_5' + str(i) + '.txt', 'w') as f:
			f.write('GO')
		...
		for knn_train_samples in knn_train_samples_:
			...
			with gcsfs.open('step_6' + str(i) + '_' + str(knn_train_samples) + '.txt', 'w') as f:
				f.write('GO')

	print(datum)
	df = pd.DataFrame([datum])

	with gcsfs.open('printing.txt', 'w') as f:
			f.write('GO')
    
	with gcsfs.open(os.path.join(FLAGS.save_dir, FLAGS.index + '.csv'), 'w') as gcs_file:
		df.to_csv(gcs_file, index=False)

	with gcsfs.open('saving_and_finishing.txt', 'w') as f:
			f.write('GO')
